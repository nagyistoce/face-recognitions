    ...
    def frame_to_face(self, frame, face_id, save_face, recognize_face):
        ...
        if self.face is not None:
            if save_face:
                ...
                # TODO: sortierung checken, gibt noch nicht korrekte ERG wieder
                self.final_predict = 500 #sorted(self.id_dict.items(), key=lambda (k,v): v[1])[0][0]
                log.debug('FINAL predict %s', self.final_predict)
